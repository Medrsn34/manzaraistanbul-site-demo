...
# Apply translations using regex to ensure whole word/phrase matching where appropriate or just direct replace.
# We sort by length descending to prevent partial replacements (e.g. replacing "Yatırım" before "Yatırım Fırsatı")
for tr_text, en_text in sorted(translations.items(), key=lambda x: len(x[0]), reverse=True):
